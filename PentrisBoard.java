...
//	}
	...
	public void rotateShape(Shape shape, int x, int y) {
		removeShapeFromBoard(shape, x, y);
//		if(isRotatePossible(shape,x,y))
			shape=shapeOp.rotate90(shape);
			while(y+shape.getShape()[0].length >board[0].length) {
				y--;
			}
	...
	public boolean isPsblLeft(Shape shape, int x, int y) {
		boolean isPsbl=true;
		for(int i=0; i<shape.getShape().length; i++) {

			if(y>0 && !board[x+i][y-1].equals("-"))
				isPsbl=false;
		}
		return isPsbl;
	}
	public boolean isPsblRight(Shape shape, int x, int y) {
		boolean isPsbl=true;
		for(int i=0; i<shape.getShape().length; i++) {

			if(y<shape.getShape()[0].length-1 && !board[x+i][y+1].equals("-"))
				isPsbl=false;
		}
		return isPsbl;
	}
	...
	public boolean isRotatePossible(Shape shape, int x, int y) {
		...
		for(int i=0; i<shapeOp.rotate90(shape).getShape().length; i++) {
			for(int j=0; j<shapeOp.rotate90(shape).getShape()[0].length; j++) {
					...
					if(!shapeOp.rotate90(shape).getShape()[i][j].equals("-") && !board[i][y+j].equals("-") )
						isPossible=false;
			}
		}
			return isPossible;
	...
	public boolean isMoveRightPossible(Shape shape, int x, int y) {
		...
		for(int i=0; i<shape.getShape()[0].length; i++) {
			if(y<board[0].length && !shape.getShape()[i][shape.getShape()[0].length-1].equals("-")
					&& !board[i][y+shape.getShape()[0].length].equals("-"))
				isPossible=false;
